...
def get_nex_position(direction, row, col):
    if direction == 'U':
        return (row - 1, col)
    if direction == 'D':
        return (row + 1, col)
...
for cmd in commands:
    next_player_row, next_player_col = get_nex_position(cmd, player_row, player_col)

    if not is_inside(rows, columns, next_player_row, next_player_col):
        won = True
    ...
    next_bunnies = get_next_bunnies(bunnies, rows, columns)
